...
export async function launchAudit(){
 ...
 await context.route('**/*',async route=>{
  ...
  if(process.env.AUDIT_CDN_CACHE){
   const root=process.env.AUDIT_CDN_CACHE;const manifest=JSON.parse(readFileSync(root+'/manifest.json','utf8'));const asset=manifest[url.href];
   if(asset)return route.fulfill({contentType:url.pathname.endsWith('.css')?'text/css':'application/javascript',body:readFileSync(root+'/'+asset.file)});
  }
