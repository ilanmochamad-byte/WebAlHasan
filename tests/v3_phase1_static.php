...
foreach(['KatalogRepository.php','KatalogService.php','V3Exception.php'] as $name){$file=$root.'/app/V3/'.$name;$check(!preg_match('/\bDELETE\s+FROM\b/i',file_get_contents($file)),'Tidak ada hard delete '.basename($file));}
...
$api=file_get_contents($root.'/api/v1/index.php');$phase2=is_file($root.'/database/migrations/014_v3_fase2_pelanggaran.sql');
$check($phase2?str_contains($api,"\$method === 'POST' && \$path === '/v3/pelanggaran'"):!preg_match('~\$method\s*===\s*\'POST\'.*/v3/~',$api),$phase2?'Mutasi V3 hanya hadir bersama paket Fase 2':'Tidak ada mutasi V3 API');
exit($fail?1:0);
