...
if(($argv[1]??'')==='cleanup'){
 ...
 if(isset($m['year'])){$db->execute_query("DELETE FROM audit_logs WHERE entity_type='tahun_ajaran' AND entity_id=?",[$m['year']]);$db->execute_query('DELETE FROM tahun_ajaran WHERE id=?',[$m['year']]);}
...
if(is_file($file)){
 ...
 if(!isset($m['year'])){$year=2090;while($db->execute_query('SELECT id FROM tahun_ajaran WHERE tahun=?',["$year/".($year+1)])->num_rows)$year++;$m['year']=master_data_service()->saveYear(['tahun'=>"$year/".($year+1),'semester'=>'Genap']);file_put_contents($file,json_encode($m,JSON_PRETTY_PRINT));}
 echo "Manifest tersedia, termasuk semester nonaktif sintetis.\n";exit;
}
...
$year=2090;while($db->execute_query('SELECT id FROM tahun_ajaran WHERE tahun=?',["$year/".($year+1)])->num_rows)$year++;$m['year']=$s->saveYear(['tahun'=>"$year/".($year+1),'semester'=>'Genap']);
