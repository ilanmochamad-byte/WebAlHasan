...
foreach($outbox as $notification){$payload=implode(' ',array_map('strval',$notification));$json=json_decode((string)$notification['data_json'],true);if(str_contains($payload,$childName)||str_contains($payload,'SBX Kategori Rahasia')||str_contains($payload,$description)||str_contains($payload,'SBX lokasi privat')||str_contains($payload,'SBX saksi rahasia')||!is_array($json)||(array_diff(array_keys($json),['type'])!==[] && !(array_keys($json)===['tipe','publikasi_id'] && $json['tipe']==='v3_publikasi' && is_int($json['publikasi_id']) && $json['publikasi_id']>0))){$privacy=false;}}
