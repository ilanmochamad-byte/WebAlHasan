...
final class KonselingService
{
    ...
    public function createCase(array $user,array $input):array
    {
        ...
        return $this->repo->transaction(function()use($user,$actorId,$data,$assignment,$hash):array{
            ...
            foreach($data['pelanggaran_ids'] as $violationId){
                ...
                if($data['kerahasiaan']==='Rahasia'&&$this->repo->activeViolationPublicationInFamily($violationId,$data['santri_id']))throw new V3Exception('Tarik publikasi pelanggaran aktif sebelum menautkannya ke kasus Rahasia.',409);
    ...
    public function correctCase(array $user,int $id,array $input):array
    {
        ...
        return $this->repo->transaction(function()use($actorId,$id,$probe,$capacity,$version,$reason,$purpose,$privacy,$key,$hash):array{
            ...
            if($privacy==='Rahasia'&&$this->repo->one("SELECT p.id FROM v3_publikasi p LEFT JOIN v3_konseling_sesi ss ON ss.id=p.sesi_id WHERE (p.kasus_id=? OR ss.kasus_id=?) AND p.ditarik_pada IS NULL LIMIT 1",[$id,$id])!==null)throw new V3Exception('Tarik seluruh publikasi aktif dengan alasan sebelum mengubah kasus menjadi Rahasia.',409);
            if($privacy==='Rahasia'&&$this->repo->caseHasActiveLinkedViolationPublication($id,(int)$probe['santri_id']))throw new V3Exception('Tarik publikasi pelanggaran tertaut sebelum mengubah kasus menjadi Rahasia.',409);
    ...
    public function addLinks(array $user,int $id,array $input):array
    {
        ...
        return $this->repo->transaction(function()use($actorId,$case,$id,$ids,$recommendationIds,$reason,$key,$hash):array{
            ...
            $created=[];foreach($ids as $violationId){if($this->repo->violationForLink($violationId,(int)$case['santri_id'],(int)$case['tahun_ajaran_id'],true)===null)throw new V3Exception('Pelanggaran tautan tidak sah atau bukan milik santri yang sama.',422);if($this->repo->violationChainLinkedToCase($violationId,$id))throw new V3Exception('Pelanggaran ini atau revisi sebelumnya sudah ditautkan ke kasus.',409);if($locked['kerahasiaan']==='Rahasia'&&$this->repo->activeViolationPublicationInFamily($violationId,(int)$case['santri_id']))throw new V3Exception('Tarik publikasi pelanggaran aktif sebelum menautkannya ke kasus Rahasia.',409);$created[]=$this->repo->insertLink($violationId,$id,null,$reason,$actorId);}
