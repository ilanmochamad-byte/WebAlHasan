...
## Bukti produksi (hosting cPanel, 11–15 September 2026)
...
Smoke test 14 September 2026 melanjutkan dengan kasus **Internal** pada santri yang sama: murobi terkait membacanya di daftar dan detail, menerima notifikasi `v3_konseling_dibuka` dan `v3_konseling_sesi`, serta menyimpan catatan murobi tingkat kasus — sementara kasus Rahasia tetap tidak muncul pada daftar murobi yang sama. Transisi `Dibuka → Selesai` ditolak "Transisi status kasus tidak sah" tanpa menaikkan versi; penjadwalan sesi memindahkan kasus ke Dalam Pendampingan secara otomatis; sesi diselesaikan lewat web dengan rencana tindak lanjut tetap utuh; penutupan `Selesai` menghasilkan versi 3 dan timeline tiga peristiwa yang tetap terbaca murobi.

Smoke test 15 September 2026 menambah siklus pelanggaran–poin dan penolakan lintas cakupan: pelanggaran #8 dicatat (`Agregat 4 · ledger 4 · selisih 0`), murobi di luar cakupan memperoleh daftar kosong dan `403` pada ID tebakan `v3_pelanggaran_detail.php?id=8`, detail terbaca rapi pada lebar ponsel, lalu pembatalan beralasan mengembalikan poin ke `Agregat 2 · ledger 2 · selisih 0` dengan riwayat revisi tetap utuh. Post-check `bin/v3_phase3_verify.php` di hosting kembali lulus 34 pemeriksaan `exit=0` — kali ini terhadap data konseling nyata, bukan tabel kosong.

Belum diuji di produksi: pembatalan kasus beserta penutupan otomatis sesi yang masih terjadwal, revisi kasus, koreksi sesi dan penjadwalan ulang, penyimpanan catatan murobi tingkat sesi, rekomendasi (satu-satunya rekomendasi produksi berstatus tidak berlaku), penolakan orang tua, serta post-check `v3_verify` dan `v3_phase2_verify` sesudah data smoke. Rinciannya di [bukti audit](audit-claude-code.md) bagian 10.
