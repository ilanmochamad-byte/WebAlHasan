...
try{$detail=v3_konseling_service()->show($currentUser,(int)$id);$timeline=v3_konseling_service()->timeline($currentUser,(int)$id);}catch(V3Exception $exception){Denial::render('Kasus tidak ditemukan atau tidak dapat dicetak.',$exception->getMessage());}
